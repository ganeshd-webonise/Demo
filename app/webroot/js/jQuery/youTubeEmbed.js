(function($){
	...
	$.fn.youTubeEmbed = function(settings){
		...
		$.get(youtubeAPI,{'q':settings.videoID},function(response){
			...
			elements.container = $('<div>').addClass('flashContainer')
			...
			elements.control = $('<div>').addClass('controlDiv play')
							   .appendTo(elements.container);

            var link = $('<a>')
                       .attr('href',data['player']['default'])
                       .attr('target','_blank')
                       .text(data['title']);

            elements.title = $('<div>').addClass('titleBar')
                             .html(link)
                             .appendTo(elements.container);
			// If the user wants to show the progress bar:

			if(settings.progressBar){
				elements.progress =	$('<div>').addClass('progressBar')
									.appendTo(elements.container);

				elements.elapsed =	$('<div>').addClass('elapsed')
